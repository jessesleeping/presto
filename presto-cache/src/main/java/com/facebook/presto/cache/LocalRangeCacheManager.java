        ...
        implements CacheManager
{
    ...
    private final Map<Path, CacheRange> persisted = new ConcurrentHashMap<>();
    // a localing cache only to control the lifecycle of persisted
    ...
    public LocalRangeCacheManager(CacheConfig cacheConfig, CacheStats stats, ExecutorService cacheFlushExecutor, ExecutorService cacheRemovalExecutor)
    {
        ...
        this.directory = new Path(cacheConfig.getBaseDirectory());
        ...
        File target = new File(directory.toUri());
        if (!target.exists()) {
            try {
    ...
    @Override
    public boolean get(FileCacheKey key, byte[] buffer, int offset)
    {
        boolean result = read(key, buffer, offset);
        if (result) {
    ...
    @Override
    public void put(FileCacheKey key, Slice data)
    {
        ...
        cacheFlushExecutor.submit(() -> {
            Path newPathHint = new Path(directory.toUri() + "/" + randomUUID() + EXTENSION);
            if (!write(key, copy, newPathHint)) {
                log.warn("%s Fail to persist cache %s with length %s ", Thread.currentThread().getName(), newPathHint, key.getLength());
            }
            stats.addInMemoryRetainedBytes(-copy.length);
        });
    }

    private boolean read(FileCacheKey key, byte[] buffer, int offset)
    {
        if (key.getLength() <= 0) {
            // no-op
            return true;
        ...
        CacheRange cacheRange = persisted.get(key.getPath());
        if (cacheRange == null) {
            return false;
        }
        ...
        readLock.lock();
        try {
            Map<Range<Long>, LocalCacheFile> diskRanges = cacheRange.getRange().subRangeMap(Range.closedOpen(key.getOffset(), key.getLength() + key.getOffset())).asMapOfRanges();
        ...
        try (RandomAccessFile file = new RandomAccessFile(new File(cacheFile.getPath().toUri()), "r")) {
            file.seek(key.getOffset() - cacheFile.getOffset());
            file.readFully(buffer, offset, key.getLength());
            return true;
        }
        catch (IOException e) {
    ...
    private boolean write(FileCacheKey key, byte[] data, Path newFilePath)
    {
        Path targetFile = key.getPath();
        persisted.putIfAbsent(targetFile, new CacheRange());

        LocalCacheFile previousCacheFile;
        LocalCacheFile followingCacheFile;

        CacheRange cacheRange = persisted.get(targetFile);
        if (cacheRange == null) {
        ...
        Lock writeLock = persisted.get(targetFile).getLock().writeLock();
        writeLock.lock();
        try {
            RangeMap<Long, LocalCacheFile> cache = persisted.get(targetFile).getRange();
            ...
            implements RemovalListener<Path, Boolean>
    {
        ...
        public void onRemoval(RemovalNotification<Path, Boolean> notification)
        {
            ...
            CacheRange cacheRange = persisted.remove(path);
            if (cacheRange == null) {
                return;
            }
